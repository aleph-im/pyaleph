...
def test_minimum_credit_cost_per_hour_for_instance(
    session_factory: DbSessionFactory,
    fixture_product_prices_aggregate_in_db,
    fixture_settings_aggregate_in_db,
):
    """Test that instance execution costs meet minimum 1 credit per hour."""
    content = {
        "time": 1701099523.849,
        "rootfs": {
            "parent": {
    ...
    with session_factory() as session:
        ...
        # Find the execution cost
        execution_cost = next((d for d in details if d.type == "EXECUTION"), None)
        assert execution_cost is not None

        # Verify the cost per hour rounds up to at least MIN_CREDIT_COST_PER_HOUR
        cost_per_hour = Decimal(execution_cost.cost_credit) * Decimal(HOUR)
