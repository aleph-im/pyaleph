...
from aleph.chains import bsc, ethereum, nuls, nuls2, substrate, cosmos, avalanche, tezos
...
async def test_incoming():
    assert len(INCOMING_WORKERS) == 4  # 3 verifiers are included by default
    assert "BSC" in INCOMING_WORKERS.keys()
    assert INCOMING_WORKERS["BSC"] is bsc.bsc_incoming_worker
