...
def upgrade() -> None:
    op.execute(
        ...
        BEGIN
            ...
            ON CONFLICT (type, status, sender, owner, channel, payment_type)
        ...
        BEGIN
            ...
            WHERE type = '' AND status = COALESCE(p_status, '')
              AND sender = '' AND owner = '' AND channel = '' AND payment_type = '';
            ...
            UPDATE message_counts SET count = count - 1
            WHERE type = '' AND status = COALESCE(p_status, '')
              AND sender = COALESCE(p_sender, '') AND owner = '' AND channel = ''
              AND payment_type = '';
            ...
            IF p_owner IS NOT NULL AND p_owner != '' THEN
                UPDATE message_counts SET count = count - 1
                WHERE type = '' AND status = COALESCE(p_status, '')
                  AND sender = '' AND owner = p_owner AND channel = ''
                  AND payment_type = '';
            END IF;
        END;
        $$ LANGUAGE plpgsql;
