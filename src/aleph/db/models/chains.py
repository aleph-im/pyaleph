...
class ChainTxDb(Base):
    ...
    @classmethod
    def from_pending_tx(cls, pending_tx: PendingTx) -> "ChainTxDb":
        """
        Builds a chain tx object from a queued pending tx MQ message.
        :param pending_tx: Pending TX object as received from the message queue.
        """

        return cls(
            hash=pending_tx.hash,
            chain=pending_tx.chain,
            height=pending_tx.height,
            datetime=pending_tx.datetime,
            publisher=pending_tx.publisher,
