...
def get_total_costs_for_address_grouped_by_message(session: DbSession, address: str, payment_type: Optional[PaymentType] = PaymentType.hold):
    total_prop = (
        AccountCostsDb.cost_hold
        if payment_type == PaymentType.hold
        else AccountCostsDb.cost_stream
    ...
    select_stmt = (
        select(AccountCostsDb.item_hash, func.sum(total_prop), id_field)
        .select_from(AccountCostsDb)
        .where(
            (AccountCostsDb.owner == address)
            & (AccountCostsDb.payment_type == payment_type)
        )
        .group_by(AccountCostsDb.item_hash)
